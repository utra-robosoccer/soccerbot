...
void Dynamixel_SetGoalPosition(Dynamixel_HandleTypeDef* hdynamixel, double goalAngle){
	...
	int normalized_value = (int)(goalAngle / MAX_ANGLE * 1023); // maximum angle of 1023
...
void Dynamixel_SetGoalVelocity(Dynamixel_HandleTypeDef* hdynamixel, double goalVelocity){
	...
	if(hdynamixel -> _isJointMode){
		normalized_value = (int)(goalVelocity / MAX_VELOCITY * 1023);
	}
	else{
		normalized_value = (int)(goalVelocity / MAX_VELOCITY * 2047);
	}
...
void Dynamixel_GetVelocity(Dynamixel_HandleTypeDef* hdynamixel){
	...
	// Read data from motor
	uint16_t retVal = Dynamixel_DataReader(hdynamixel, 0x26, 2);
...
void Dynamixel_GetLoad(Dynamixel_HandleTypeDef* hdynamixel){
	...
	hdynamixel -> _lastLoad = retVal / 1023 * 100;
}
...
void Dynamixel_GetVoltage(Dynamixel_HandleTypeDef* hdynamixel){
	...
	// Read data from motor
	uint8_t retVal = (uint8_t)Dynamixel_DataReader(hdynamixel, 0x2A, 1);
...
void Dynamixel_GetTemperature(Dynamixel_HandleTypeDef* hdynamixel){
	...
	hdynamixel -> _lastTemperature = (uint8_t)Dynamixel_DataReader(hdynamixel, 0x2B, 1);
}
...
uint8_t Dynamixel_IsRegistered(Dynamixel_HandleTypeDef* hdynamixel){
	...
	return((uint8_t)Dynamixel_DataReader(hdynamixel, 0x2C, 1));
}
...
uint8_t Dynamixel_IsMoving(Dynamixel_HandleTypeDef* hdynamixel){
	...
	return((uint8_t)Dynamixel_DataReader(hdynamixel, 0x2E, 1));
}
...
uint8_t Dynamixel_IsJointMode(Dynamixel_HandleTypeDef* hdynamixel){
	...
	// Read data from motor
	uint16_t retValCW = Dynamixel_DataReader(hdynamixel, 0x06, 2);
	uint16_t retValCCW = Dynamixel_DataReader(hdynamixel, 0x08, 2);

	// Parse data and write it into motor handle
	hdynamixel -> _isJointMode = ((retValCW + retValCCW ) != 0);
...
void Dynamixel_TestAll(Dynamixel_HandleTypeDef** arrHdynamixel, uint8_t arrSize){
	 ...
	 * Pass: Motor(s) rotate through over 360 degrees
	 */
	for(int i = 0; i < arrSize; i++){
	...
	HAL_Delay(1500);
	for(int i = 0; i < arrSize; i++){
		Dynamixel_SetGoalVelocity(arrHdynamixel[i], MIN_VELOCITY);
	}
	HAL_Delay(1500);
