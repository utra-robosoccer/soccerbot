...
#include <soccer_team_communication/team_data.h>
...
ros::Publisher pub;
static soccer_team_communication::local_model team_models[PLAYER_NUM];

static void parser_rcv(char* buff, soccer_team_communication::team_data& msg)
{
	uint8_t robotID = 0;
	uint8_t min = ROBOT_ID_MIN;
	uint8_t max = PLAYER_NUM;
	soccer_team_communication::local_model model;
...
{
	//ros setup
	ros::init(argc, argv, "soccer_team_communication_server");
	ros::NodeHandle n;
	...
	pub = n.advertise<soccer_team_communication::team_data>("team_data", 1);
	...
	{
		char buff[1024] = "\0";
		soccer_team_communication::team_data msg;
