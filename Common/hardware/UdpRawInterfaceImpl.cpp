...
err_t UdpRawInterfaceImpl::udpBind(
    struct udp_pcb *pcb,
...
void UdpRawInterfaceImpl::udpRecv(
    struct udp_pcb *pcb,
    udp_recv_fn recv,
    void *recv_arg
) const
...
err_t UdpRawInterfaceImpl::udpConnect(
    struct udp_pcb *pcb,
    const ip_addr_t *ipaddr,
    u16_t port
) const
...
err_t UdpRawInterfaceImpl::udpSend(
    struct udp_pcb *pcb,
    struct pbuf *p
) const
{
...
struct pbuf* UdpRawInterfaceImpl::pbufAlloc(
        pbuf_layer layer,
        u16_t length,
        pbuf_type type
) const
...
u16_t UdpRawInterfaceImpl::pbufCopyPartial(
...
err_t UdpRawInterfaceImpl::pbufTake(
        struct pbuf *buf,
        const void *dataptr,
        u16_t len
) const
