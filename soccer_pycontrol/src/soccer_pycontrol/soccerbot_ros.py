...
class SoccerbotRos(Soccerbot):
    ...
    def __init__(self, pose: Transformation, useFixedBase=False, useCalibration=True):
        ...
        self.head_yaw_freq = rospy.get_param("head_yaw_freq", 0.01 if torch.cuda.is_available() else 0.005)
        self.head_yaw_freq_relocalizing = rospy.get_param("head_yaw_freq_relocalizing", 0.02 if torch.cuda.is_available() else 0.005)

        #: Frequency for the head's while searching and relocalizing yaw (up and down movement)
        self.head_pitch_freq = rospy.get_param("head_pitch_freq", 0.01 if torch.cuda.is_available() else 0.005)
        self.head_pitch_freq_relocalizing = rospy.get_param("head_pitch_freq_relocalizing", 0.02 if torch.cuda.is_available() else 0.005)
    ...
    def apply_head_rotation(self):
        if self.robot_state.status in [self.robot_state.STATUS_DETERMINING_SIDE, self.robot_state.STATUS_PENALIZED]:
            self.configuration[Joints.HEAD_1] = math.cos(self.head_step * self.head_pitch_freq) * (math.pi / 4)
