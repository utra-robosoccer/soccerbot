...
void update_motor_position(MotorPort *port, uint8_t id, uint16_t angle) {
  angle %= 0x3ff; // max range for ax12
  uint8_t data[2] = {angle & 0xff, (angle>>8) & 0xff};
  uint8_t dataLen = 2;
  uint8_t addr = 30;
  dynamixel_write_v1(port, id, addr, data, dataLen);
}
 ...
 */
void motor_write_p1(GPIO_TypeDef *uartPort, uint16_t dirPin, UART_HandleTypeDef huart, uint8_t id, uint8_t addr, uint8_t* data, uint8_t dataLen) {
  ...
  // set write direction
  HAL_GPIO_WritePin(uartPort, dirPin, GPIO_PIN_SET);

  // Write to Motor on UART
  HAL_UART_Transmit(&huart, txBuf, packetLen, 1000);
}
