...
from soccer_object_detection.model import (
    CNN,
    Label,
    find_batch_bounding_boxes,
    init_weights,
)
from soccer_object_detection.my_dataset import initialize_loader
from soccer_object_detection.train import Trainer
from soccer_object_detection.util import draw_bounding_boxes, stream_image
...
def train_model(load_model=None, num_features=16):
    experiment = {
        ...
        "train_class_weight": [0.18, 0.02, 0.80],  # BALL, ROBOT, OTHER
...
def webcam():
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            img = draw_bounding_boxes(img, bbxs[Label.ROBOT.value], (0, 0, 255))
            img = draw_bounding_boxes(img, bbxs[Label.BALL.value], (255, 0, 0))
            stream_image(img, wait=25, scale=4)
