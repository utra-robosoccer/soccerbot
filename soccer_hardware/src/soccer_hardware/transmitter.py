...
class Transmitter(Thread):
    ...
    def start(self, *args, **kwargs):
        with self._ser._motor_lock:
            jx_servo_util.uart_transact(self._ser, [1600, 1, 15] * 13, CMDS.PID_COEFF, RWS.WRITE)  # push initial PID gains
            jx_servo_util.uart_transact(self._ser, [1700] * 13, CMDS.MAX_DRIVE, RWS.WRITE)  # push initial maximum drive (out of 4096)
    ...
    def _send_packet_to_mcu(self, goal_motor_angles):
        ...
        with self._ser._motor_lock:  # NOTE: possibly replace with acquire-release with timeout
            ...
            for idx, v in jx_goal_angles:
                flat_jx_goal_angles[int(idx)] = int(v * ANGLE2JX_POT_VOLTS)
            ...
            flat_gobilda_goal_angles = [0] * int(np.amax(gobilda_goal_angles[:, 0]) - GOBILDA_IDX_BASE + 1)
            for idx, v in gobilda_goal_angles:
                flat_gobilda_goal_angles[int(idx - GOBILDA_IDX_BASE)] = int(v * ANGLE2GOBILDA_PWM)
