...
bool readFromSensor(imu::MPU6050& imu_data, uint32_t* num_samples){
    bool success = false;

    imu_data.Read_Accelerometer(); // _IT // these made more sense when using the RTOS, need to rethink the whole program design to do this
    ...
    ++*num_samples;
    if(*num_samples % 16 == 0){
        imu_data.Read_Gyroscope(); // _IT // these made more sense when using the RTOS, need to rethink the whole program design to do this
        success = true;
    }
