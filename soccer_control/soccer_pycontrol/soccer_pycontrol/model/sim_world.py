...
class SimWorld:
    ...
    def __init__(self, model_dir: Optional[str] = None, scene_name: Optional[str] = None, robot_model:str = "bez2", keyframe: str = "stand", pose: Optional[Transformation] = None ):
        # If model_dir is not provided, use the current directory
        if model_dir is None:
            model_dir = expanduser("~") + f"/ros2_ws/src/soccerbot/soccer_description/{robot_model}_description/mjcf"
        else:
            model_dir = os.path.join(os.path.dirname(__file__) + "/model/")
        if scene_name is None:
            scene_name = "scene_"+robot_model+".xml"
        ...
        if pose is None:
            if keyframe == "stand":
                self.T = Transformation(position=[0, 0, 0.3975], euler=[0,0, 0]) # TODO these should be in config
                self.set_T_world_site("torso", self.T.matrix)
            elif keyframe == "fallen_front":
                self.T = Transformation(position=[0, 0, 0.09], euler=[0, 1.57, 0])
                self.set_T_world_site("torso", self.T.matrix)
            elif keyframe == "fallen_back":
                self.T = Transformation(position=[0, 0, 0.115], euler=[0, -1.57, 0])
                self.set_T_world_site("torso", self.T.matrix)
            elif keyframe == "fallen_side_left":
                self.T = Transformation(position=[0, 0, 0.14], euler=[0, 0, -1.57])
                self.set_T_world_site("torso", self.T.matrix)
            elif keyframe == "fallen_side_right":
                self.T = Transformation(position=[0, 0, 0.140], euler=[0, 0, 1.57])
                self.set_T_world_site("torso", self.T.matrix)
            elif keyframe == "float":
    ...
    def set_T_world_ball(self, T_world_siteTarget: Transformation) -> None:
        self.set_T_world_site("ball", T_world_siteTarget.matrix)
    ...
    def set_T_world_fbase(self, T: np.ndarray, site_name:str = "root") -> None:
        """
        ...
        """
        joint = self.data.joint(site_name)
    ...
    def set_T_world_site(self, site_name: str, T_world_siteTarget: np.ndarray, ) -> None:
        ...
        T_world_fbase = self.get_T_world_fbase(site)
        T_world_site = self.get_T_world_site(site_name)
        T_site_fbase = np.linalg.inv(T_world_site) @ T_world_fbase



        self.set_T_world_fbase(T_world_siteTarget @ T_site_fbase, site_name=site)
    ...
    def get_T_world_fbase(self, site_name:str = "root") -> np.ndarray:
        """
        Gets the transformation from world to floating base frame.
        """
        data = self.data.joint(site_name).qpos
        quat = data[3:]
        pos = data[:3]
...
if __name__ == "__main__":
    ...
    sim.step()
    sim.set_T_world_site("torso", np.eye(4))
