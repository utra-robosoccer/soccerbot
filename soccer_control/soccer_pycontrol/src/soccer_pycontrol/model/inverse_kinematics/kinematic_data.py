...
class KinematicData:
    ...
    def __init__(
        ...
        if robot_model == "sigmaban":
            self.urdf_model_path = expanduser("~") + f"/catkin_ws/src/soccerbot/soccer_description/bez1_description/urdf/{robot_model}/robot.urdf"
            #: Height of the robot's torso (center between two arms) while walking
            self.walking_torso_height = 0.5
        else:
            self.urdf_model_path = (
                expanduser("~") + f"/catkin_ws/src/soccerbot/soccer_description/{robot_model}" f"_description/urdf/{robot_model}.urdf"
            )
            self.walking_torso_height = parameters["walking_torso_height"]  # 0.5
        ...
        self.thigh_length = np.linalg.norm(motor_offsets["right_hip_pitch"] - motor_offsets["right_knee"])
        self.tibia_length = np.linalg.norm(motor_offsets["right_knee"] - motor_offsets["right_ankle_pitch"])
        self.torso_to_right_hip = Transformation(position=(motor_offsets["right_hip_yaw"] + [0, 0, 0]), euler=[0, 0, 0])
