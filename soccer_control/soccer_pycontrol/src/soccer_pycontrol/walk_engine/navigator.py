...
class Navigator:
    ...
    def walk(self, target_goal: Union[Transformation, List]):
    ...
    def walk_pose(self, target_goal: Transformation):
        ...
        position = self.bez.sensors.get_pose().position  # can use self.foot_step_planner.trajectory.get_p_world_CoM(t)
        while (
            self.position_error(position[:2], target_goal.position[:2]) > self.error_tol
        ):  # self.bez.sensors.get_pose() #TODO about 20% or 40% error
            position = (
                self.bez.sensors.get_pose().position
            )  # self.foot_step_planner.robot.get_T_world_trunk()  # can use self.foot_step_planner.trajectory.get_p_world_CoM(t)
            x_error = target_goal.position[0] - position[0]
            y_error = target_goal.position[1] - position[1]
            # TODO replace with pure pursuit
            # TODO make  a 2d unit test

            dx = self.nav_x_pid.update(position[0])
            dy = self.nav_y_pid.update(position[1])
            print(dx, " ", dy, " ", x_error, " ", y_error)
    ...
    @staticmethod
    def position_error(goal_loc: np.ndarray, curr_loc: np.ndarray):
