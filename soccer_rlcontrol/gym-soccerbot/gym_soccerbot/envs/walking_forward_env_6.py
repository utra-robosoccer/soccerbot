...
class WalkingForwardV6(gym.Env):
    ...
    _MAX_ANG_VEL = 500.0  # LSM6DSOX
    _MAX_LIN_ACC = 2.0 * 9.81  # LSM6DSOX
    ...
    if _CONTROL_MODE == Control_Mode.VELOCITY:
        ...
        if _ENABLE_HANDS:
            ...
            action_space = spaces.Box(
                low=_joint_limit_low[Joints.LEFT_LEG_1 : Joints.HEAD_1],
                high=_joint_limit_high[Joints.LEFT_LEG_1 : Joints.HEAD_1],
                dtype=DTYPE,
            )

    # Observation Space
    _OBSERVATION_DIM = _JOINT_DIM + _JOINT_DIM + _IMU_DIM + _ORN_DIM + _HEIGHT_DIM + _FEET_DIM - (_DIM_SUB_HANDS * 2)
    imu_limit = np.concatenate((np.array([_MAX_LIN_ACC] * int((_IMU_DIM) / 2)), np.array([_MAX_ANG_VEL] * int((_IMU_DIM) / 2))))
    # pose_limit = np.array([3.] * _POSE_DIM)
    orn_limit = np.array([1.0] * _ORN_DIM)
    height_limit_high = np.array([0.5] * _HEIGHT_DIM)
    height_limit_low = np.array([0.1] * _HEIGHT_DIM)
    ...
    observation_limit_high = np.concatenate(
        (
            joint_limit[_DIM_SUB_HANDS : Joints.HEAD_1],
            vel_limit * 2,
            imu_limit,
            orn_limit,
            height_limit_high,
    ...
    observation_limit_low = np.concatenate(
        (
            -joint_limit[_DIM_SUB_HANDS : Joints.HEAD_1],
            -vel_limit * 2,
            -imu_limit,
    ...
    def _imu(self):
        ...
        imu_noise = np.concatenate(
            (
                self.np_random.normal(0, self._IMU_LIN_STDDEV, int(self._IMU_DIM / 2)),
    ...
    def _feet(self):
        ...
        for i in range(len(locations)):  # 5% chance of incorrect reading
            locations[i] *= np.sign(
                self.np_random.uniform(low=-self._FEET_FALSE_CHANCE, high=1 - (self._FEET_FALSE_CHANCE)),
                dtype=self.DTYPE,
            )
    ...
    def _standing_poses(cls, np_random=None, scale=0.1):
        standing_poses = [0.0] * (cls._JOINT_DIM + 2)
        ...
        if np_random is not None:
            standing_poses[cls._DIM_SUB_HANDS : cls._JOINT_DIM] = (
                standing_poses[cls._DIM_SUB_HANDS : cls._JOINT_DIM]
                + scale
                * np_random.uniform(-0.0, 1.0, np.size(standing_poses[cls._DIM_SUB_HANDS : cls._JOINT_DIM]))
    ...
    def _standing_poses2(cls, np_random=None, scale=0.6):
        ...
        if np_random is not None:
            ...
            standing_poses[cls._DIM_SUB_HANDS : cls._JOINT_DIM] = np.clip(
                standing_poses[cls._DIM_SUB_HANDS : cls._JOINT_DIM], cls._joint_limit_low, cls._joint_limit_high
            )
        return standing_poses
    ...
    def reset(self):
        ...
        p.resetBasePositionAndOrientation(self.soccerbotUid, [0, 0, self._STANDING_HEIGHT], [0.0, 0.0, 0.0, 1.0])
        ...
        self.imu_bias = np.concatenate(
            (
                self.np_random.normal(0, self._IMU_LIN_STDDEV_BIAS, int(self._IMU_DIM / 2)),
