...
class WalkingForwardV4(gym.Env):
    ...
    _MAX_ANG_VEL = 250.0  # MPU6050
    _MAX_LIN_ACC = 2.0  # MPU6050
    ...
    def _feet(self):
        ...
        for i in range(len(locations)):  # 5% chance of incorrect reading
            locations[i] *= np.sign(
                self.np_random.uniform(low=-self._FEET_FALSE_CHANCE, high=1.0 - (self._FEET_FALSE_CHANCE)),
                dtype=self.DTYPE,
            )
    ...
    def _standing_poses(cls, np_random=None, scale=0.6):
        ...
        if np_random is not None:
            standing_poses[0 : cls._JOINT_DIM] = (
                standing_poses[0 : cls._JOINT_DIM]
                + scale
                * np_random.uniform(-0.0, 1.0, np.size(standing_poses[0 : cls._JOINT_DIM]))
                * (cls._joint_limit_high - standing_poses[0 : cls._JOINT_DIM])
                + scale
                * np_random.uniform(-0.0, 1.0, np.size(standing_poses[0 : cls._JOINT_DIM]))
                * (cls._joint_limit_low - standing_poses[0 : cls._JOINT_DIM])
            )
            standing_poses[0 : cls._JOINT_DIM] = np.clip(standing_poses[0 : cls._JOINT_DIM], cls._joint_limit_low, cls._joint_limit_high)
    ...
    def step(self, action):
        ...
        joints_pos = np.clip(joints_pos, -self.joint_limit, self.joint_limit)
