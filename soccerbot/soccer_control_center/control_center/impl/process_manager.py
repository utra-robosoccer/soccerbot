...
class SupervisorImpl(ProcessManager):
    def __init__(self, hostname='localhost', port=6001):
        self.server = ServerProxy('http://{}:{}/RPC2'.format(hostname, port))
        self.supervisor = self.server.supervisor
        self.hostname = hostname
        self._config = current_app.config[hostname]
    ...
    def _ssh_scp_client(self):
        ...
        if self.hostname == 'localhost':
            ssh_client.connect(self.hostname)
        else:
            ssh_client.connect(self.hostname, username=self._config['ssh_username'],
    ...
    def _add_process_section(self, section, process, config_parser):
        ...
        log_path = self._config['process_log_dir'] + log_file_name
    ...
    def _write_to_file_and_validate(self, config_parser, group):
        ...
        remote_new_file = self._config['process_config_dir'] + filename
        ...
        while retries_performed < RETRY_COUNT:
            ...
            try:
                ...
                ssh.exec_command('mv {} {}'.format(remote_tmp_file, remote_new_file))
                success = True
                break
    ...
    def _write_to_file_bulk(self, config_parser_dict):
        ...
        while retries_performed < RETRY_COUNT:
            ...
            try:
                ssh, scp = self._ssh_scp_client()
                scp.put(local_files, self._config['process_config_dir'])
    ...
    def _purge_config(self, group):
        ssh = None
        dep_file = self._config['process_config_dir'] + group + '.conf'
        try:
            ssh, _ = self._ssh_scp_client()
            ssh.exec_command('rm {}'.format(dep_file))
        except Exception as e:
    ...
    def _purge_all_config(self):
        folder = self._config['process_config_dir']
        ssh = None
        try:
            ssh, _ = self._ssh_scp_client()
            ssh.exec_command('rm {}*'.format(folder))
        except Exception as e:
            LOG.error('Supervisor config rm error, {}'.format(e.message))
        finally:
            if ssh:
