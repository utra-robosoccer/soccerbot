...
float Camera::VerticalFOV() {
    return Camera::resolution_y / sqrt(pow(Camera::resolution_x ,2.0) + pow(Camera::resolution_y ,2.0));
}
...
Point3 Camera::FindFloorCoordinate(int pos_x, int pos_y) {
    ...
    float tmp3 [4]= {static_cast<float>(Camera::pose.orientation.w),static_cast<float>(Camera::pose.orientation.x),static_cast<float>(Camera::pose.orientation.y),static_cast<float>(Camera::pose.orientation.z)};
